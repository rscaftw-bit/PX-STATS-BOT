...
BACKFILL_LIMIT = int(os.getenv("BACKFILL_LIMIT", "500"))  # hoeveel berichten teruglezen bij start
...
# ========== POKEDEX NAAMVERTALING (1–1025) ==========
POKEDEX_FILE = "pokedex.json"

def _load_pokedex():
    # Probeer lokaal bestand
    if os.path.exists(POKEDEX_FILE):
        with open(POKEDEX_FILE,"r",encoding="utf-8") as f:
            return json.load(f)
    # Val terug op publieke lijst (Bulbasaur→Pecharunt). Wordt lokaal gecached.
    try:
        from urllib.request import urlopen
        with urlopen("https://raw.githubusercontent.com/fanzeyi/pokemon.json/master/pokedex.json", timeout=10) as r:
            data=json.load(r)
        mapping={p["id"]:p["name"]["english"] for p in data}  # key=int id, val=English name
        # Bewaar als string keys om JSON-compat zeker te zijn
        mapping_str = {str(k): v for k, v in mapping.items()}
        with open(POKEDEX_FILE,"w",encoding="utf-8") as f: json.dump(mapping_str,f,ensure_ascii=False)
        print(f"[POKEDEX] created local file with {len(mapping_str)} entries")
        return mapping_str
    except Exception as e:
        print("[POKEDEX ERR]", e)
        return {}
...
def _translate_pnumber(name:str)->str:
    # Alleen exact p### wordt vertaald; gewone namen blijven ongewijzigd
    m=re.match(r"p(\d+)$",name.strip(),re.I)
    if m:
        n=m.group(1)
        return POKEDEX.get(n, name)
    return name
...
def _normalize_pname(name: str) -> str:
    # Verwijder spatie tussen p en nummer (p 816 → p816)
...
# ========== PARSER (fix: invasion→rocket, strikte shiny) ==========
...
# ========== BACKFILL ==========
async def backfill_from_channel(limit=BACKFILL_LIMIT):
...
async def on_ready():
    try:
        ...
        load_events()
        await backfill_from_channel(BACKFILL_LIMIT)
